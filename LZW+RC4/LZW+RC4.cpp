// LZW+RC4.cpp: определяет точку входа для консольного приложения.
//
...
/* Количество битов в коде (размерность ЭЛЕМЕНТА) */
#define BITS                       12
/* Максимальное значение кода */
#define MAX_CODE                   ( ( 1 << BITS ) - 1 )
/*
Размер словаря (количество ЭЛЕМЕНТОВ)
Размер строки таблицы должен быть простым числом, которое
несколько больше, чем 2 ** BITS.
*/
#if BITS == 20
...
void print_ratios(char *input, char *output)
{
	...
	printf("\nВходной размер:        %ld bytes\n", input_size);
	printf("Выходной размер:       %ld bytes\n", output_size);
	if (output_size == 0)
		output_size = 1;
	printf("Соотношение: %d%%\n", ratio);
}
...
/* Стек для декодирования, этот массив содержит декодируемые строки */
...
void RC4_e(char *argv10, char *argv30, char *argv40)
{
	...
	printf("\nШифрование ключом '%s' выполнено успешно!\n\n\n\n", argv40);
}
...
void RC4_d(char *argv10, char *argv20, char *argv50)
{
	...
	printf("\nРасшифровка ключом '%s' выполнена успешно!\n", argv50);
}
...
int main(int argc, char *argv[])
{
	...
	if (argc < 5)
	{
		std::cerr << "\nНужно больше (золота) аргументов!\nЗагляни в .bat'ник.\n\n\n\n";
		system("pause");
		return 0;
	}
	if (argc > 6)
	...
	if (argv[1][0] == 'e' || argv[1][0] == 'E')
	{
		...
		//		char *outFile = argv[3]; //РАБОТАЕТ, но outFile хранит лишь ссылку, а не argv[3]
		//		outFile.pop_back();
		//		outFile.erase(outFile.size() - 1);
		//		outFile[strlen(outFile) - 4] = '\0'; //РАБОТАЕТ, но изменяется argv[3], а не outFile
		if (argc < 5)
		{
			std::cerr << "\nНужно больше (золота) аргументов!\nЗагляни в .bat'ник.\n\n\n\n";
			system("pause");
		...
		printf("\nАрхивирование было успешно завершено за %i мс.\n", search_time);
		printf("\nРазмер словаря в элементах: %i (%i бит)\n", TABLE_SIZE, BITS);
	...
	else if (argv[1][0] == 'd' || argv[1][0] == 'D')
	{
		...
		if (input == NULL)
			fatal_error("Ошибка открытия %s для чтения\n", argv[3]);

		// открытие выходного файла для записи
		output = fopen(argv[4], "wb");
		if (output == NULL)
			fatal_error("Ошибка открытия %s для записи\n", argv[4]);
		...
		printf("\nРазархивирование было успешно завершено за %i мс.\n\n\n\n", search_time);
	};
